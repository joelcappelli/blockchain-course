...
class Block:
    ...
    creator_id: int
    prev_hash: str = "0"
    timestamp: str = datetime.datetime.utcnow().strftime("%d/%m/%Y %H:%M:%S")
    nonce: str = 0
...
def setup():
    ...
    return PyChain([Block(
        record=Record(sender = "Genesis", receiver = "Genesis", amount = 0.0),
        creator_id=42,
        prev_hash= "0"
    )])
...
# YOUR CODE HERE
record_sender = st.text_input("Record (Sender): ")

# @TODO:
# Add an input area where you can get a value for `receiver` from the user.
# YOUR CODE HERE
record_receiver = st.text_input("Record (Receiver): ")

# @TODO:
# Add an input area where you can get a value for `amount` from the user.
# YOUR CODE HERE
record_amount = st.number_input("Record (Amount): ")
...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_block = Block(
        record=Record(sender = record_sender, receiver = record_receiver, amount = record_amount),
        creator_id=42,
